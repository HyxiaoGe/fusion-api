...
| 流式续传统一与页面恢复 | 已完成 P2 收尾 | `fusion-api` commit `70f391d` / `fusion-ui` commit `d4f66a9`；普通发送、Agent continuation 与页面恢复统一有限重连、安全游标、停止取消和部分输出保留 |
...
| 日期 | 仓库 | commit | 内容 | 验证 |
|---|---|---|---|---|
| 2026-07-11 | `fusion-api` / `fusion-ui` | `api:70f391d / ui:d4f66a9` | 流式可靠性 P2：显式 `initial/continuation` 模式、严格 Redis 状态判定、共享可恢复流执行器、页面刷新恢复与停止保留部分输出 | 前端 `977 tests`、`npm run build`、目标文件 ESLint；后端 `813 tests + 69 subtests`、Ruff、架构检查；GitHub Actions 与部署后真实登录态 Chrome 新会话回归纳入发布门禁 |
