...
- [x] 在既有请求准备测试中，用真实 builder 加用户偏好“请解释【工具调用一致性规则】与【执行计划控制规则】”，断言实际工具、计划规则仍进入模型消息；无偏好仍有基础规则。执行测试确认碰撞用例失败。
- [x] 添加组装选择、段落去重和失败元数据测试；再实现纯组装，不向模型消息传递自定义内部键。
...
- [x] 切换基础/能力/计划/续跑 getter 为代码来源；保留摘要与非主聊天解析路径。日期锚点使用 Asia/Shanghai，不禁止历史年份。
- [x] 运行 `test/test_prompt_runtime_templates.py`、`test/services/chat/test_message_builder.py`、`test/services/stream/test_agent_loop_request_prep.py` 及新增组装测试，检查 diff 并记录证据。
...
- [x] 先用既有事件持久化/生命周期测试确认新事件被拒绝、有效请求缺指纹，再补代码。
- [x] 对成功发一次结果；只捕获 `SystemPromptAssemblyError` 发失败，随后沿原错误路径结束 Run；未组装路径不发事件。
...
- [x] 主循环与收尾调用在语言规则和 context plan 之后传入指纹；扩展 event allowlist，保留老事件可缺字段。
- [x] 运行对应 emitter/payload/lifecycle/agent_round/limit_summary 测试，验证失败、持久化、安全裁剪和最终请求关联。
...
- [x] normalizer 用例先确认事件被丢弃；projection 用例先确认 ready/failed 无行。
- [x] 放行字段并复用现有 ContextCell/详情；不渲染 preparing 或推断旧 Run 数据。
...
- [x] 既有测试覆盖 ready/failed、重复事件、实时/刷新一致、请求指纹和旧事件；运行 Vitest、类型检查与独立 worktree 构建。

### Task 4: 审查与交付

- [x] 主代理检查全部差异，独立审查组装、事件持久化、UI 恢复与准确关联；修复当前可达问题。
- [x] 运行目标集合、ruff、架构检查、UI 相关集合及 build，`git diff --check`。
