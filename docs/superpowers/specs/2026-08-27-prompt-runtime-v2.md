...
- 天气活动判断只能基于返回字段给条件化结论，不得推断路况、安全性或舒适度；只有昼夜预报时不得把白天改写成上午或下午。
- 航班和高铁结果同时存在时，正文必须覆盖两类，不得遗漏任一类型或使用 Markdown 表格重复结构化卡片。
...
- 两个失败都发生在工具成功、结构化卡片完整之后，说明 ToolMessage 级约束对当前模型不够稳定。修复选择在产品结果出现后恢复高优先级静态约束，同时保持初始 Run Prompt 精简和既有确定性收口逻辑不变。
