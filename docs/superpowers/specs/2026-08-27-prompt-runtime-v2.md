...
- `5d7085d8` 部署后，越界天气正文已被确定性校验拒绝，证明安全边界生效；该次模型回答因 `weather_fact_mismatch` 且无法修整，进入确定性天气兜底。
- 原天气兜底只汇总四天预报和通用雨具建议，不读取原用户问题，因此没有回答“上午骑行的条件是否满足”。安全不等于完成用户任务，该结果仍判定为验收失败。
- 天气兜底现在只在用户明确询问已知活动、细分时段和结果内日期时读取原用户文本：选择对应日期，陈述返回天气，明确昼夜粒度无法确认细分时段，并以“避开降水”作为显式条件说明满足与否；不输出活动适宜性或改期建议。普通天气兜底保持原行为。
