...
def build_default_queries() -> dict[str, str]:
    mib = "1024/1024"
    return {
        "api_cpu_percent": 'sum(rate(container_cpu_usage_seconds_total{name="fusion-api"}[5m])) * 100',
        ...
        "postgres_cpu_percent": 'sum(rate(container_cpu_usage_seconds_total{name="postgres"}[5m])) * 100',
        ...
        "redis_cpu_percent": 'sum(rate(container_cpu_usage_seconds_total{name="middleware-redis"}[5m])) * 100',
        ...
        "nginx_cpu_percent": 'sum(rate(container_cpu_usage_seconds_total{name="nginx-proxy"}[5m])) * 100',
        ...
        "litellm_cpu_percent": 'sum(rate(container_cpu_usage_seconds_total{name="litellm-proxy"}[5m])) * 100',
