...
| xAI | `xai` | 原生 | Grok 系列 |
| Xiaomi（小米 MiMo） | `xiaomi_mimo` | LiteLLM 原生 | MiMo 系列 |
| MiniMax | `minimax` | LiteLLM 原生 | M2 系列 |
| Moonshot（月之暗面） | `moonshot` | LiteLLM 原生 | Kimi 系列 |

需要自定义 `api_base` 的提供商：qwen、volcengine（从数据库凭证中读取 `base_url`）。
