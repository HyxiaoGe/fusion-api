...
WEATHER_RESULT_ROUND_PROMPT = (
    ...
    "不得评价温度或风力是否适合、可接受或舒适，不得声称天气会影响活动体验、路面状况或安全。"
    "如果用户询问是否适合某项活动，应给出基于已返回字段的条件化结论，但不要直接使用“适合”“不适合”“建议”“不建议”评价活动。"
    "只说明返回事实、时间粒度边界和用户条件是否满足；例如用户希望避雨时，只能表述为“如果你的条件是避雨，"
    "本次返回的雨天预报不满足这一条件”。不要追加活动装备、路线、危险或体验方面的推断。"
...
MIXED_TRAVEL_RESULT_ROUND_PROMPT = (
    ...
    "只比较参考价格和班次计划时长，分别使用各自实际返回的字段找出本次候选中的省钱和快速选择，再给出跨类型结论。"
    "不得比较总出行时间、接驳便利性、值机、安检或候车，不得补充结构化结果中没有的机场、车站或其他实体。"
    "即使行程卡片只展示航班，也不能省略高铁；卡片不是已覆盖高铁正文的理由。"
    "正文固定包含总体结论、航班摘要、高铁摘要和事实边界，共四个短段落；不使用 Markdown 表格，"
    "也不得把某一类型的字段拼接到另一类型的班次上。"
)
