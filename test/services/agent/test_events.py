...
class AgentEventModelTests(unittest.TestCase):
    def _common(self):
        return dict(run_id="r1", step_id="s1", tool_call_id=None,
    ...
    def test_run_completed_finish_reason_enum(self):
        for fr in ("stop", "limit_reached"):
            RunCompleted(type="run_completed", total_steps=1, total_tool_calls=0,
                         finish_reason=fr, **self._common())
...
class AgentEventExportTests(unittest.TestCase):
    def test_all_event_classes_importable(self):
        ...
        classes = [
            AgentEventBase,
            RunStarted, StepStarted, ToolCallStarted, ToolCallDelta,
            ToolCallCompleted, StepCompleted, RunLimitReached,
            RunInterrupted, RunFailed, RunCompleted,
        ]
